...
export type AssetFile = { kind: "bytes"; data: Buffer; mediaType: string };
...
export async function saveAsset(
  ...
  if (
    ...
    asset.generationBatchIndex !== undefined &&
    asset.generationBatchIndex !== null
  ) {
    const existing = await db.asset.findUnique({
      where: {
        generationJobId_generationBatchIndex: {
          generationJobId: asset.generationJobId,
          generationBatchIndex: asset.generationBatchIndex,
        },
      },
      select: RECORD_SELECT,
    });
  ...
  const stored = await (deps.storeImage ?? storeAssetImage)(
    id,
    image,
    mediaType,
  );
  try {
    ...
    const duplicateGenerationOutput =
    ...
    if (duplicateGenerationOutput) {
      const concurrent = await db.asset.findUnique({
        where: {
          generationJobId_generationBatchIndex: {
            generationJobId: asset.generationJobId!,
            generationBatchIndex: asset.generationBatchIndex!,
...
export async function readAssetFile(
  ...
  if (row.storageKey) {
    const stored = await readAssetObject(row.storageKey);
    if (stored)
      return { kind: "bytes", data: stored, mediaType: row.mediaType };
  }
  if (row.image)
    return {
      kind: "bytes",
      data: Buffer.from(row.image),
      mediaType: row.mediaType,
    };
  if (row.imageUrl) {
    const legacy = await fetchImageUrl(row.imageUrl);
    if (legacy)
      return { kind: "bytes", data: legacy, mediaType: row.mediaType };
  }
  return null;
}
...
export async function readAssetWithImage(
  id: string,
  ownerId: string,
): Promise<AssetWithImage | null> {
  if (!/^[a-f0-9-]{36}$/.test(id)) return null;
  const row = await db.asset.findFirst({
    where: { id, ownerId: requireOwnerId(ownerId) },
    select: IMAGE_SELECT,
...
export async function listAssetSlices(
  parentId: string,
  ownerId: string,
): Promise<AssetRecord[]> {
...
export async function readAssetFiles(
  ...
  const knownBytes = rows.reduce(
    (total, row) => total + (row.byteSize ?? 0),
    0,
  );
