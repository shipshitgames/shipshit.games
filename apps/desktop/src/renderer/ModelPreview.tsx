// three.js GLTF previewer for the Studio 3D pane. The optimize pipeline
...
export function ModelPreview({ src, label = "3D model preview" }: ModelPreviewProps) {
  ...
  return (
    <div className="model-stage" aria-label={label}>
      ...
      {loading && !error && <div className="model-status">decoding…</div>}
      {error && <div className="model-status is-err">preview failed: {error}</div>}
    </div>
  );
}
