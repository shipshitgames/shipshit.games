...
All changes are in the working tree, uncommitted. `dist/` is gitignored (Vercel rebuilds from source).
...
- **Redeploy `games/pactfall`** so the fix goes live (changes are source-only, uncommitted).
