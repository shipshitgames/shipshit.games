...
## Session 2: Reusable asset QA primitives and manifest CLI

### System Flow

```mermaid
flowchart LR
  M["Product-owned asset-qa.json\ntargets + thresholds"] --> C["assetgen asset-qa check\nread-only + deterministic"]
  M --> R["assetgen asset-qa repair\nexplicit mutation"]
  C --> D["Actionable human/JSON diagnostics"]
  R --> P["decode -> crop -> pad -> rematte -> encode"]
  P --> A["Atomic replace when bytes changed"]
  A --> V["Re-run declared checks"]
```

### Affected Components

- `packages/assetgen/src/asset-qa`: public RGBA, PAM/WebP I/O, metric,
  transform, manifest, check, and repair APIs.
- `packages/assetgen/src/commands/asset-qa.ts` and `src/cli.ts`: explicit
  `asset-qa check|repair` commands.
- `packages/assetgen/asset-qa.schema.json` and fixture: product manifest
  contract.
- `packages/assetgen/ASSET-QA.md` / `README.md`: integration and migration
  documentation.
- `packages/assetgen/src/commands/clean-sprites.ts`: reuses the consolidated
  alpha-bounds primitive.

### What Was Done

- Added byte-exact PAM RGBA encode/decode and shared temporary-directory,
  atomic-output, libwebp encode/decode, and WebP payload-kind helpers.
- Consolidated alpha bounds/margins, border counts, neighbor/luma scans,
  dark-edge and dark-fringe rematting, metrics, crop, and horizontal-cell
  padding as public primitives.
- Added a strict v1 manifest parser/schema with root-contained paths,
  deterministic target ordering, thresholds, explicit repair pipelines, and
  target selection.
- Added read-only `asset-qa check` and explicit `asset-qa repair` commands with
  stable JSON reports, actionable diagnostics, non-zero failure exits, atomic
  replacement, byte-identical no-op writes, and post-repair validation.
- Added focused unit and e2e coverage for PAM, temp cleanup, metrics, crop/pad,
  rematting, lossless detection, schema/parser safety, deterministic/no-write
  checks, idempotent repairs, and CLI JSON/exit behavior.
- Documented the product integration contract and the exact migration of the
  three July Deadrot repair scripts and their three duplicated helper modules.
- Inspected the Deadrot scripts read-only; no Deadrot checkout files or GitHub
  state were changed.

### Key Decisions

- Product repositories own paths, selected atlas bounds, and acceptance
  thresholds; assetgen contains only generic primitives and execution logic.
- Check and repair are separate CLI actions so CI cannot accidentally mutate
  assets.
- Repair operation order is fixed and reviewable: decode, crop, pad, rematte,
  encode, atomic replace, validate.
- WebP mutation uses libwebp PAM plus `-exact`; repair asserts the requested
  lossless/lossy payload before replacement.
- Check-only targets may omit repair declarations; repair processes only
  declared mutations and rejects explicitly selected check-only targets.

### Files Changed

- `.agents/SESSIONS/2026-07-09.md`
- `packages/assetgen/ASSET-QA.md`
- `packages/assetgen/README.md`
- `packages/assetgen/asset-qa.schema.json`
- `packages/assetgen/e2e/asset-qa.e2e.test.ts`
- `packages/assetgen/package.json`
- `packages/assetgen/src/asset-qa/{index,io,manifest,rgba,types}.ts`
- `packages/assetgen/src/asset-qa/{io,manifest,rgba}.test.ts`
- `packages/assetgen/src/cli.ts`
- `packages/assetgen/src/commands/asset-qa.ts`
- `packages/assetgen/src/commands/clean-sprites.ts`
- `packages/assetgen/test-fixtures/asset-qa/manifest.json`

### Verification

- `bun run test`: 670 passed, 0 failed.
- `bun run typecheck`: passed.
- `bun run lint`: passed (`check-design`).
- `bun run e2e`: 122 passed, 0 failed.
- Strict Ajv 2020 compilation/fixture validation for
  `asset-qa.schema.json`: passed.
- `git diff --check`: passed.

### Mistakes And Fixes

- Initial focused tests could not resolve dependencies because the isolated
  worktree had no `node_modules`; installed the locked Bun workspace with
  `bun install --frozen-lockfile` without changing the lockfile.
- The first crop exclusivity expression in the JSON Schema worked in
  permissive validators but failed Ajv strict compilation; rewrote it as a
  strict-compatible `oneOf` and added mode-specific rematte option validation.

### Next Steps

- In Deadrot, add the product-owned `asset-qa.json` using the documented exact
  target/value transfer, add the read-only check to the asset gate, and keep
  repair manual.
- Once the product manifest passes, remove the three one-off fix scripts and
  their duplicated helper modules in the Deadrot repository.
