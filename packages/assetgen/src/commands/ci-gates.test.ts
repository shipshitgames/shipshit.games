...
describe("assetgen ci-gates plan", () => {
  test("accepts only an exact declaration of every discovered target", async () => {
    await withTempDir(async (dir) => {
      ...
      await writeFile(
        ...
        "// GENERATED by `assetgen codegen --game deadlane` — do not edit by hand.\nexport {};\n",
        "utf8",
      );
  ...
  test("leaves package-native generated modules to their native integrity gate", async () => {
    await withTempDir(async (dir) => {
      const assetsDir = join(dir, "assets");
      const gamesRoot = join(dir, "games");
      await mkdir(assetsDir, { recursive: true });
      await mkdir(join(gamesRoot, "scourge-survivors", "src"), {
        recursive: true,
      });
      await writeFile(
        join(gamesRoot, "scourge-survivors", "src", "assets.generated.ts"),
        "// Generated by the downstream package native asset index.\nexport {};\n",
        "utf8",
      );
      const configPath = await writeManifest(
        dir,
        manifest({
          mode: "native-only",
          reason: "The package uses its native generated module.",
          targets: { indexes: [], atlases: [], codegen: [] },
        }),
      );
